...
std::weak_ptr<KdEffekseerObject> KdEffekseerManager::Play(
	const std::string& effName, const DirectX::SimpleMath::Vector3& pos, const float size, const float speed, bool isLoop, const DirectX::SimpleMath::Vector3& _rotate)
{
	PlayEfkInfo info;
...
void KdEffekseerManager::StopEffect(const std::string& name, const Math::Vector3& pos)
{
	auto foundItr = m_nowEffectPlayList.begin();

	while (foundItr != m_nowEffectPlayList.end())
	{
		// 座標が一致するならエフェクトを止める
		if (foundItr->get()->GetPos() == pos)
		{
			if (foundItr->get()->IsLoop())
			{
				foundItr->get()->SetLoop(false);
			}
			m_efkManager->StopEffect(foundItr->get()->GetHandle());
			return;
		}
	...
	if (foundItr == m_nowEffectPlayList.end()) { return; }
}
