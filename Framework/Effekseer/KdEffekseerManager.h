...
public:
	...
	// Effekseerエフェクト再生
	std::weak_ptr<KdEffekseerObject> Play(const std::string& effName, const DirectX::SimpleMath::Vector3& pos, const float size = 1, const float speed = 1, const bool isLoop = true, const DirectX::SimpleMath::Vector3& _rotate = Math::Vector3::Zero);

	void StopAllEffect();
	void StopEffect(const std::string& name);

	void StopEffect(const std::string& name, const Math::Vector3& pos);
...
#define EffekseerPath "Asset/Data/Effect/Effekseer/"